...
def surrounded(line, idx):
    ...
    exactly_three = True
    if idx == 3:
        if is_capital(line[7]):
           exactly_three = False
    elif idx == l-4:
        if is_capital(line[l-8]):
            exactly_three = False
    else:
        if is_capital(line[idx+4]) or is_capital(line[idx-4]):
            exactly_three = False
    return exactly_three


arr = string.split()  # arr[1250][80]
ans = []
# xXXXoXXXx 인 o 찾기
for r in range(1250):
    for c in range(3, 77):
        if is_capital(arr[r][c]):
            continue
        if surrounded(arr[r], c):
            ans.append(arr[r][c])
print(''.join(ans))
# linkedlist
